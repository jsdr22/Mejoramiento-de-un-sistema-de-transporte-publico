...
# Preparar los datos de tiempos para graficar
if hash_dispositivo in datos_lagos_v1 and hash_dispositivo in datos_127_v1:
    tiempos_lagos_v1 = datos_lagos_v1[hash_dispositivo]
    tiempos_127_v1 = datos_127_v1[hash_dispositivo]
else:
...
# Función para graficar solo puntos sin unirlos con líneas
def graficar_puntos(tiempos_lagos, tiempos_127, color, label, grosor):
    tiempos = tiempos_lagos + tiempos_127
    y = [0] * len(tiempos_lagos) + [1] * len(tiempos_127)
    plt.scatter(tiempos, y, color=color, label=label, zorder=3, s=grosor)

# Función para graficar líneas entre los puntos
def graficar_lineas(tiempos_lagos, tiempos_127, color, label, grosor):
    tiempos = sorted(tiempos_lagos + tiempos_127)
    y = [0 if tiempo in tiempos_lagos else 1 for tiempo in tiempos]
    plt.plot(tiempos, y, color=color, label=label, linewidth=grosor)

# Graficar los datos del sistema principal y complementario como puntos
#graficar_puntos(tiempos_lagos_v1, tiempos_127_v1, '#009BA8', 'Sistema Principal', grosor=40)
graficar_puntos(tiempos_lagos_v2, tiempos_127_v2, 'g', 'Sistema Complementario', grosor=20)

# Graficar los datos del tercer código como línea continua para la traza teórica
graficar_lineas(tiempos_lagos_teo, tiempos_127_teo, '#ADD8E6', 'Traza Teórica', grosor=6)

# Configurar las etiquetas en el eje y
plt.yticks([0, 1], ['lagos', '127'], fontsize=18)

# Añadir título a los ejes y al gráfico
plt.xlabel('Tiempo (H:M)', fontsize=20)
plt.ylabel('Estación de Transmilenio', fontsize=22)
plt.title('Traza de movimiento sistema complementario Samsung A71 background', fontsize=22)

# Establecer los ticks del eje x basados en la traza teórica
# Asumiendo que `tiempos_lagos_teo` y `tiempos_127_teo` están ya definidos y contienen objetos datetime
tiempos_teo = sorted(set(tiempos_lagos_teo + tiempos_127_teo))

# Establecer las marcas de tiempo y las etiquetas en el eje x
plt.xticks(tiempos_teo, [t.strftime('%H:%M') for t in tiempos_teo], rotation=90)

# Establecer un formato para las marcas de tiempo
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))

# Establecer los ticks del eje x con un tamaño de fuente más grande
plt.xticks(fontsize=14)
# Autoajustar las fechas para que encajen bien en el eje
plt.gcf().autofmt_xdate()
...
plt.gca().add_artist(legend)  # Esto es necesario para que la leyenda no sea reemplazada por el texto
plt.text(0.01, 0.88, 'Precisión sistema ', fontsize=14, transform=plt.gca().transAxes)
plt.text(0.01, 0.85, 'principal ', fontsize=14, transform=plt.gca().transAxes)
plt.text(0.01, 0.82, 'de 46%', fontsize=14, transform=plt.gca().transAxes)
